...
namespace MyDelegateEvent.Event
{
    ...
    public class Cat
    {
        ...
        private List<IObject> _IObjectList = new List<IObject>();
        public void AddObject(IObject iObject)
        {
            this._IObjectList.Add(iObject);
        }

        public void RemoveObject(IObject iObject)
        {
            this._IObjectList.Remove(iObject);
        }

        public void MiaoPattern()
        {
            Console.WriteLine("{0} MiaoPattern", this.GetType().Name);
            if (this._IObjectList.Count() > 0)
                this._IObjectList.ForEach(t => t.DoAction());
        }
