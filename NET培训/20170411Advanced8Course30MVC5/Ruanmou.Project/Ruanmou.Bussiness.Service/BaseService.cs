...
namespace Ruanmou.Bussiness.Service
{
    public abstract class BaseService : IBaseService
    {
        ...
        public PageResult<T> QueryPage<T>(Expression<Func<T, bool>> funcWhere, int pageSize, int pageIndex) where T : class
        {
            PageResult<T> result = new PageResult<T>()
            {
                DataList = this.Context.Set<T>().Where<T>(funcWhere).Skip((pageIndex - 1) * pageSize).Take(pageSize).ToList(),
                PageIndex = pageIndex,
                PageSize = pageSize,
