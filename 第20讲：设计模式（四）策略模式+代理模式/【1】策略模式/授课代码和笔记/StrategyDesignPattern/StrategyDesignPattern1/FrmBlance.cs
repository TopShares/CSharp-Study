...
namespace StrategyDesignPattern1
{
    public partial class FrmBlance : Form
    {
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            switch (cbbCalType.SelectedIndex)
            {
                case 0:
                    subTotal = Convert.ToDouble(txtUnitPrice.Text) * Convert.ToDouble(this.txtQuantity.Text);
                    break;
                case 2:
                    subTotal = Convert.ToDouble(txtUnitPrice.Text) * Convert.ToDouble(this.txtQuantity.Text)*0.8;
                    break;
                case 3:
                    subTotal = Convert.ToDouble(txtUnitPrice.Text) * Convert.ToDouble(txtQuantity.Text) * 0.7;
                    break;
                case 4:
                    subTotal = Convert.ToDouble(txtUnitPrice.Text) * Convert.ToDouble(txtQuantity.Text) * 0.6;
                    break;
            }
            //结算列表
